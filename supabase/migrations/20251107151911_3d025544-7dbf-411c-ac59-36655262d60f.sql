-- Atualizar função para considerar apenas tarefas com progresso >= 1%
...
-- Atualizar função de análise financeira para considerar apenas tarefas com progresso >= 1%
DROP FUNCTION IF EXISTS public.get_task_financial_analytics(integer);
...
DECLARE
    ...
    eff_score NUMERIC;
BEGIN
    -- Calcular custos planejados vs reais (APENAS COM PROGRESSO >= 1%)
    SELECT 
    ...
    -- Calcular tempo planejado vs real (APENAS COM PROGRESSO >= 1%)
    SELECT 
    ...
    -- Contar tarefas dentro/fora do orçamento (APENAS COM PROGRESSO >= 1%)
    SELECT 
    ...
    -- Contar tarefas no prazo/atrasadas (APENAS COM PROGRESSO >= 1%)
    SELECT 
...
-- Atualizar função de tarefas com desvios para considerar apenas com progresso >= 1%
DROP FUNCTION IF EXISTS public.get_top_deviation_tasks(integer, integer);
