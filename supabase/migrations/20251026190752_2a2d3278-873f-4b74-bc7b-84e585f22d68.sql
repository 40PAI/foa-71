-- Corrigir função get_dashboard_geral_data com status enum válidos e lógica de encarregado correta
...
BEGIN
  ...
  -- Buscar nome do usuário para encarregado
  ...
  SELECT json_build_object(
    ...
    'kpis_gerais', (
      SELECT json_build_object(
        ...
        'projetos_ativos', COUNT(*) FILTER (WHERE status IN ('Em Andamento', 'Atrasado')),
    ...
    'tarefas_resumo', (
      SELECT json_build_object(
        ...
        'em_andamento', COUNT(*) FILTER (WHERE status = 'Em Progresso'),
    ...
    'requisicoes_resumo', (
      SELECT json_build_object(
        'total', COUNT(*),
        'pendentes', COUNT(*) FILTER (WHERE status_fluxo = 'Pendente'),
