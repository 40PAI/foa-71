-- Garantir que notificações não bloqueiam a eliminação de projetos no futuro
ALTER TABLE public.notificacoes
  DROP CONSTRAINT IF EXISTS notificacoes_projeto_id_fkey;
...
-- Recriar a função RPC usada pela aplicação para limpar completamente os dados do projeto
...
BEGIN
  ...
  BEGIN
    -- Limpar dependências indiretas antes das tabelas principais
    ...
    DELETE FROM public.pagamentos_recebimentos
    WHERE contrato_cliente_id IN (SELECT id FROM public.contratos_clientes WHERE projeto_id = project_id)
    ...
    WHERE projeto_origem_id = project_id
       ...
       OR movimentacao_origem_id IN (
         SELECT id FROM public.materiais_movimentacoes
         WHERE projeto_origem_id = project_id OR projeto_destino_id = project_id
       );
    ...
    -- Limpar dados operacionais do projeto
    ...
    DELETE FROM public.requisicoes WHERE id_projeto = project_id OR projeto_destino_id = project_id;

    -- Remover ligações em dados mestres reutilizáveis
    ...
    -- Apagar estruturas do projeto depois das dependências
...
GRANT EXECUTE ON FUNCTION public.delete_project_safely(integer) TO authenticated;