-- Corrigir detect_financial_discrepancies para usar colunas correctas de requisicoes
-- requisicoes usa: valor (não valor_total), id_projeto (não projeto_id), 
-- status_fluxo (não status), categoria_principal (não categoria)
...
BEGIN
  FOREACH cat IN ARRAY categorias_padrao LOOP
    ...
    -- Gastos de requisições aprovadas (usando colunas correctas)
    SELECT COALESCE(SUM(r.valor), 0)
    INTO v_req
    FROM requisicoes r
    WHERE r.id_projeto = p_project_id 
      AND r.status_fluxo = 'Aprovado'
...
-- Corrigir calculate_integrated_financial_progress para usar colunas correctas de requisicoes
...
BEGIN
  ...
  -- Adicionar gastos de requisições aprovadas (usando colunas correctas)
  SELECT 
  ...
  FROM requisicoes r
  WHERE r.id_projeto = p_project_id AND r.status_fluxo = 'Aprovado';
