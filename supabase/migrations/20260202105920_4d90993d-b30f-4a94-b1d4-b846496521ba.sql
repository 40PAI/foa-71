-- Corrigir detect_financial_discrepancies para usar coluna 'valor' em vez de 'saida'
...
-- Corrigir calculate_integrated_financial_progress para usar 'valor' em vez de 'saida'
...
BEGIN
  ...
  -- Agregar gastos de movimentos_financeiros por categoria normalizada (usando 'valor' para saídas)
  SELECT 
  ...
  SELECT 
    v_material_expenses + COALESCE(SUM(CASE WHEN normalize_financial_category(r.categoria::text) = 'Materiais' THEN r.valor_total ELSE 0 END), 0),
    v_patrimony_expenses + COALESCE(SUM(CASE WHEN normalize_financial_category(r.categoria::text) = 'Patrimonio' THEN r.valor_total ELSE 0 END), 0)
  INTO v_material_expenses, v_patrimony_expenses
  FROM requisicoes r
  WHERE r.projeto_id = p_project_id AND r.status = 'Aprovado';
  ...
  -- Buscar custos de tarefas (usando percentual_conclusao)
  SELECT 
