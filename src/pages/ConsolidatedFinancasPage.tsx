...
export function ConsolidatedFinancasPage() {
  ...
  const { data: consolidatedData, isLoading } = useConsolidatedFinancialData(selectedProjectId || 0);
    ...
    <div className="p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
      ...
      <Accordion type="multiple" className="w-full space-y-3">
        <CollapsibleFinancialSection 
          ...
          icon={Wallet}
          badge={{ text: "FOA", variant: "default" }}
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Suspense fallback={<SectionLoadingFallback rows={2} />}>
              <CategoryExpenseCard 
                ...
                totalBudget={consolidatedData?.financas?.find(f => f.categoria === "Materiais de Construção")?.orcamentado || 1000000}
                fromTasks={consolidatedData?.integrated_expenses?.material_total || 0}
                fromCentroCusto={0}
                manualExpenses={0}
              />
            </Suspense>
            <Suspense fallback={<SectionLoadingFallback rows={2} />}>
              <CategoryExpenseCard 
                ...
                icon={Users} 
                projectId={selectedProjectId} 
                totalBudget={consolidatedData?.financas?.find(f => f.categoria === "Mão de Obra")?.orcamentado || 1000000}
                fromTasks={consolidatedData?.integrated_expenses?.mao_obra_total || 0}
                fromCentroCusto={0}
                manualExpenses={0}
              />
            </Suspense>
            <Suspense fallback={<SectionLoadingFallback rows={2} />}>
              <CategoryExpenseCard 
                ...
                icon={Truck} 
                projectId={selectedProjectId} 
                totalBudget={consolidatedData?.financas?.find(f => f.categoria === "Equipamentos")?.orcamentado || 1000000}
                fromTasks={consolidatedData?.integrated_expenses?.patrimonio_total || 0}
                fromCentroCusto={0}
                manualExpenses={0}
              />
            </Suspense>
            <Suspense fallback={<SectionLoadingFallback rows={2} />}>
              <CategoryExpenseCard 
                ...
                icon={DollarSign} 
                projectId={selectedProjectId} 
                totalBudget={consolidatedData?.financas?.find(f => f.categoria === "Custos Indiretos")?.orcamentado || 1000000}
                fromTasks={consolidatedData?.integrated_expenses?.indireto_total || 0}
                fromCentroCusto={0}
                manualExpenses={0}
              />
            </Suspense>
        ...
        <CollapsibleFinancialSection 
          ...
          icon={Search} 
          badge={(consolidatedData?.discrepancies?.length || 0) > 0 ? {
