...
export function drawCoverPage(doc: any, options: CoverOptions): void {
  ...
  if (projeto) {
    ...
    if (projeto.localizacao) {
      doc.text(`Localizacao: ${projeto.localizacao}`, centerX, y, { align: 'center' });
      y += 8;
    }

    // Responsible
    if (projeto.responsavel) {
      doc.text(`Responsavel: ${projeto.responsavel}`, centerX, y, { align: 'center' });
      y += 8;
    }
  }
  ...
  if (periodo) {
    ...
    doc.setFontSize(10);
    setColor(doc, colors.medium);
    doc.text(`Periodo: ${periodo}`, centerX, y, { align: 'center' });
    y += 8;
  }
  ...
  doc.setFontSize(10);
  setColor(doc, colors.medium);
  doc.text(`Data de Geracao: ${formatDatePDF(new Date())}`, centerX, bottomY, { align: 'center' });
