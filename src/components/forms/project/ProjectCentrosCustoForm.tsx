...
export function ProjectCentrosCustoForm({ 
  ...
  const updateCentroCusto = (index: number, field: keyof ProjectCentroCusto, value: string | number) => {
    ...
    newCentros[index] = { ...newCentros[index], [field]: value };
    onCentrosCustoChange(newCentros);
  };
  ...
  const isCentroValido = (centro: ProjectCentroCusto) => {
    return centro.codigo.trim() !== "" && 
           centro.nome.trim() !== "" && 
           centro.tipo &&
           (stages.length === 0 || centro.etapa_numero !== undefined);
  };
        ...
        <div key={index} className="border rounded-lg p-4 space-y-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              {isCentroValido(centro) ? (
                <Badge variant="default" className="bg-green-500">✓ Válido</Badge>
              ) : (
                <Badge variant="destructive">✗ Campos obrigatórios</Badge>
              )}
            </div>
            <Button
              type="button"
              variant="ghost"
              size="sm"
              onClick={() => removeCentroCusto(index)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div className="flex gap-2">
                <Input
                  value={centro.codigo}
                  onChange={(e) => updateCentroCusto(index, "codigo", e.target.value)}
                  placeholder="Ex: CC-001"
                  className={!centro.codigo.trim() ? "border-red-500 focus-visible:ring-red-500" : ""}
                />
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => gerarCodigoAutomatico(index)}
            ...
            <div>
              ...
              <Select
                value={centro.tipo}
                onValueChange={(value: TipoCentroCusto) => updateCentroCusto(index, "tipo", value)}
              >
                <SelectTrigger>
            ...
            <div className="md:col-span-2">
              ...
              <Input
                value={centro.nome}
                onChange={(e) => updateCentroCusto(index, "nome", e.target.value)}
                placeholder="Ex: Material de Construção - Etapa Fundação"
                className={!centro.nome.trim() ? "border-red-500 focus-visible:ring-red-500" : ""}
              />
            </div>
            ...
            <div>
              ...
              <Select
                value={centro.etapa_numero?.toString()}
                onValueChange={(value) => updateCentroCusto(index, "etapa_numero", parseInt(value))}
              ...
              >
                ...
                <SelectContent>
                  {stages.map((stage) => (
                    <SelectItem key={stage.numero_etapa} value={stage.numero_etapa.toString()}>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateCentroCusto(index, "departamento", e.target.value)}
                placeholder="Ex: Obras, Compras"
              />
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <Input
                ...
                onChange={(e) => updateCentroCusto(index, "orcamento_mensal", parseFloat(e.target.value) || 0)}
                placeholder="0.00"
              />
