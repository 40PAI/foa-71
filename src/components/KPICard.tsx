...
export function KPICard({ title, value, subtitle, trend, alert, icon, info }: KPICardProps) {
  ...
  return (
    <Card className="relative min-w-0 h-full min-h-[5.75rem]">
      ...
      <CardContent size="sm" className="space-y-1">
        <div className="flex items-start justify-between min-w-0 gap-1">
          <div className="font-bold min-w-0 flex-1 break-words leading-tight" style={{ fontSize: "clamp(0.82rem, 1.8vw, 1.125rem)" }}>
            {value}
          </div>
        ...
        {subtitle && (
          <p className="text-muted-foreground leading-tight break-words hyphens-auto" style={{ fontSize: "clamp(0.62rem, 1.1vw, 0.75rem)" }}>
            {subtitle}
          </p>
        )}
      </CardContent>
